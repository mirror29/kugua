...
<script>
  ...
  import {
    ...
    computed: {
      ...
      async sendRequest(){
          try {
              const res = await this.$request.post('/payment/payment-show', {
                  data:{
                    Id: this.id
                  },
                  header:{
        ...
        try {
          ...
          if (res.data.code === 0) {
            const res2 = await this.$request.post("/payment/update-payment", {
              data: {
                Id: this.id,
