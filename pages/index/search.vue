<template>
	<view class="content">
		<view class="search-box">
			...
			<view class="search-btn" @click="doSearch(false)">搜索</view>
			-->
			<!-- 原样式 end -->
		</view>
		<view class="search-keyword">
			<scroll-view class="keyword-list-box" v-show="isShowKeywordList" scroll-y>
				<view class="keyword-entry" hover-class="keyword-entry-tap" v-for="row in keywordList" :key="row.keyword">
					<view class="keyword-text" @click="doSearch(row.keyword)">
						<rich-text :nodes="row.htmlStr"></rich-text>
					</view>
					<view class="keyword-img" @click="doSearch(row.keyword)">
			...
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				<view class="keyword-block" v-if="oldKeywordList.length>0">
					<view class="keyword-list-header">
						<view>历史搜索</view>
						<view>
							<image @click="oldDelete" src="/static/HM-search/delete.png"></image>
						</view>
					</view>
					<view class="keyword">
						<view v-for="(keyword,index) in oldKeywordList" @click="doSearch(keyword)" :key="index">{{keyword}}</view>
					</view>
				</view>
				<view class="keyword-block">
					<view class="keyword-list-header">
						<view>热门搜索</view>
						<view @click="hotToggle">
							<image :src="'/static/HM-search/attention'+forbid+'.png'"></image>
						</view>
					</view>
					<view class="keyword" v-if="forbid==''">
						<view v-for="(keyword,index) in hotKeywordList" @click="doSearch(keyword.keyword)" :key="index">{{keyword.keyword}}</view>
					</view>
  ...
  import List from "@/components/guess-list.vue"
    ...
    onReady(){
      ...
      async sendRequest() {
        ...
        this.defaultKeyword = searchText.data.data.keyword;
        const hot =  await this.$request.post('/search/keywords', {});
        this.hotKeywordList = hot.data.data;
        let net = ''
        ...
        uni.navigateTo({
          url: `../product/goods-list?key=${newkey}`
        });
        // this.keyword = ''
        this.isShowKeywordList = false;
			},
...
<style>
	...
	.search-box .input-box>input {width:100%;height:60upx;font-size:32upx;border:0;border-radius:60upx;-webkit-appearance:none;-moz-appearance:none;appearance:none;padding:0 4%;margin:0;background-color:#ffffff;}
	.placeholder-class {color:#9e9e9e;}
	.search-keyword {width:100%;background-color:rgb(242,242,242);}
	.keyword-list-box {height:calc(100vh - 110upx);padding-top:20upx;border-radius:20upx 20upx 0 0;background-color:#fff;}
	.keyword-entry-tap {background-color:#eee;}
	.keyword-entry {width:94%;height:80upx;margin:0 4%;font-size:30upx;color:#333;display:flex;justify-content:space-between;align-items:center;border-bottom:solid 1upx #e7e7e7;}
	...
	.keyword-entry .keyword-text {width:80%;}
	...
	.keyword-box .keyword-block .keyword-list-header {width:100%;padding:10upx 4%;font-size:27upx;color:#333;display:flex;justify-content:space-between;}
	.keyword-box .keyword-block .keyword-list-header image {width:40upx;height:40upx;}
	.keyword-box .keyword-block .keyword {width:100%;padding:3px 4%;display:flex;flex-flow:wrap;justify-content:flex-start;}
