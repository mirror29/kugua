...
export default {
  ...
  methods: {
    ...
    async getListDataFromNet(pageNum,pageSize,successCallback,errorCallback) {
        try {
					const list = await this.$request.post("/balance/income", {
						data: {
              page: pageNum,
              limit: pageSize
            },
          ...
          for (const item of list.data.data) {
            item.month = `${this.$common.formatTime(item.month).split(" ")[0].split("/")[0]} / ${this.$common.formatTime(item.month).split(" ")[0].split("/")[1]}`
          }
