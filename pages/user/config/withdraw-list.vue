  ...
  <view class="warp">
    <view class="box">
      <t-table>
        ...
        <mescroll-uni top="102" @down="downCallback" :up="upOption" @up="upCallback" @emptyclick="emptyClick" @init="mescrollInit" :tipShow = false>
            <t-tr v-for="item in list" :key="item.id" class="table-b">
              <t-td>{{item.time}}</t-td>
              <t-td>{{item.amount}}</t-td>
              <t-td v-if="item.status === 'check'">待审核</t-td>
              <t-td v-else-if="item.status === 'refuse'">已拒绝</t-td>
              <t-td v-else-if="item.status === 'payment'">支付中</t-td>
              <t-td v-else-if="item.status === 'finish'">已完成</t-td>
            </t-tr>
        </mescroll-uni>
      </t-table>
    </view>
...
export default {
  ...
  methods: {
    ...
    async getListDataFromNet(pageNum,pageSize,successCallback,errorCallback) {
        try {
					const list = await this.$request.post("/withdraws/index", {
						data: {
              page: pageNum,
              limit: pageSize
            },
...
// .t-td,.t-th{
//   border-left: none;
//   border-top: 2upx solid rgb(212, 212, 212) !important;
//   padding: 30upx 14upx;
// }
